...
public:
    ...
    void uniform(const char *name, const glm::vec2 &vec) const
        { glUniform2fv(glGetUniformLocation(m_prog, name),
            1, glm::value_ptr(vec)); }
    ...
    void uniform(const char *name, const Texture &texture, GLenum slot = GL_TEXTURE0) const {
		texture.bind(slot);
